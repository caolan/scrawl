...
exports.parseTags = function (comment) {
    ...
    if (comment.param) {
        if (!Array.isArray) {
            comment.param = comment.param ? [comment.param]: [];
        }
        comment.params = comment.param.reduce(function (params, str) {
            var match = /(?:^\{(\S+)\}\s+)?(?:([\S]+)\s*)?([\s\S]*)?/.exec(str);
            if (match) {
                var name = match[2];
                params[name] = {
                    type: match[1],
    ...
    // parse @returns tags
    if (comment.returns) {
        var match = /^\{(\S+)\}/.exec(comment.returns);
        if (match) {
...
exports.parseModules = function (paths, include_hidden, callback) {
    ...
    function (err, modules) {
        ...
        async.reduce(modules, {}, function (result, m, cb) {
            fs.readFile(m, function (err, content) {
                if (err) {
                    return cb(err);
                }
                result[m] = exports.parse(content.toString());
                cb(null, result);
            });
        }, callback);
    });
